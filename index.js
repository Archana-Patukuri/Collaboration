...
import * as THREE from "three";
 let model; let container; let camera, scene, renderer; let controls; let currentUserName = ""; 
   ...
   camera = new THREE.PerspectiveCamera( 70, window.innerWidth / window.innerHeight, 0.01, 20 );
    ...
    const light = new THREE.HemisphereLight(0xffffff, 0xbbbbff, 3);
     light.position.set(0.5, 0, 0.25);
      scene.add(light); renderer = new THREE.WebGLRenderer({ antialias: true, alpha: true });
       ...
       controls.screenSpacePanning = false;
        controls.target.set(0, 0.1, 0);
         ...
         loader.load("GLB Models/Precedent_Rooftop_Unit.glb", 
          (gltf) => { model = gltf.scene; model.position.set(0, 0, 0); scene.add(model); console.log("model loaded"); 
                      ...
                      button.style.backgroundColor = "#32c9e0";
                       tabButtons[0].style.backgroundColor = "#30f0f0";
                        ...
                        async function collaboration() {
                           ...
                           /* let ymap = ydoc.getMap("stateMap"); let value = 0; ymap.set("state", value); */ 
                           const yText = ydoc.getText("chatMessages");
                            const provider = new WebsocketProvider( "ws: localhost:5173/", "chat-room", ydoc );
                                   ...
                                   document.getElementById("share").addEventListener("click", () => { 
                                    const sessionID = getOrCreateSessionID();
                                     const sessionLink = `http:localhost:5173//${sessionID}`;
                                       ...
                                       copySessionID.addEventListener("click", () => { navigator.clipboard.writeText(sessionLink); 
                                       ...
                                       participantsName.set(currentUserName, { color: getRandomColor(), joinedAt: Date.now(), });
                                        addUserToSession(userName);
                                        } else { alert("Please enter a name!"); } }); });
                                        function addUserToSession(userName) {
                                        const li = document.createElement("li"); li.textContent = userName; 
                                       list.appendChild(li);
                                       ...
                                       const prevRotation = cameraState.get("rotation");
                                        const newPosition = { x: camera.position.x, y: camera.position.y, z: camera.position.z, };
                                        const newRotation = { x: camera.rotation.x, y: camera.rotation.y, z: camera.rotation.z, }; 
                                       if ( JSON.stringify(prevPosition) !== JSON.stringify(newPosition) || JSON.stringify(prevRotation) !== JSON.stringify(newRotation) ) {
                                        cameraState.set("position", newPosition); cameraState.set("rotation", newRotation); } 
                                       });
                                        let isUndoRedo = false; cameraState.observe(() => { 
                                       if (isUndoRedo) return;   
                                       const position = cameraState.get("position");
                                        const rotation = cameraState.get("rotation"); 
                                       if (position && rotation) {
                                        camera.position.set(position.x, position.y, position.z); 
                                       camera.rotation.set(rotation.x, rotation.y, rotation.z); controls.update(); 
                                     } 
                                       ...
                                       yText.insert(yText.length, `${currentUserName}: ${message}\n`); inputField.value = "";
                                        ...
                                        /* yText.observe(() => { document.getElementById("chat-box").innerText = yText.toString(); }); 
                                       */ yText.observe(() => { const chatBox = document.getElementById("chat-box"); chatBox.innerHTML = "";
                                           const messages = yText.toString().split("\n"); 
                                          messages.forEach((msg) => { const [userName, text] = msg.split(": "); 
                                          const userInfo = participantsName.get(userName) || { color: "#ffffff" }; 
                                          console.log(userName); const messageDiv = document.createElement("div");
                                        messageDiv.innerText = msg; messageDiv.style.color = userInfo.color; console.log(msg);
                                         
                                      messageDiv.style.textAlign = userName === currentUserName ? "right" : "left";
                                        chatBox.appendChild(messageDiv); }); });
                                        /* const undoManager = new UndoManager([cameraState], { captureTimeout: 0 });
                                        undoBtn.addEventListener("click", () => {
                                        if (undoManager.canUndo()) {
                                        console.log("undo clicked", undoManager); 
                                       isUndoRedo = true; undoManager.undo(); isUndoRedo = false; 
                                       } else {
                                        alert("Undo stack is empty");
                                        } });
                                        redoBtn.addEventListener("click", () => { 
                                       if (undoManager.canRedo()) {
                                        isUndoRedo = true; undoManager.redo(); isUndoRedo = false;
                                        } else { 
